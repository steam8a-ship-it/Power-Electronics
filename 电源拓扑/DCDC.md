...
|---|---|---|---|
| **功能** | 降压 | 升压 | 升压 / 降压，经典拓扑输出反相 |
| **关键公式** | $V_o=D V_{in}$ | $V_o=\frac{V_{in}}{1-D}$ | $V_o=-\frac{D}{1-D}V_{in}$ |
| **电压关系** | $V_o<V_{in}$ | $V_o>V_{in}$ | 可升（D＞0.5）可降 （D＜0.5）|
| **输入电流** | 脉动（斩波） | **连续** | 脉动 |
| **输出电流** | **连续** | 脉动 | 脉动 |
| **电感位置** | 输出侧 | 输入侧 | 输入与输出之间的储能环节 |
| **电感平均电流** | $\approx I_o$ | $\approx I_{in}$ | 通常高于 $I_o$ |
| **MOSFET位置** | 常见高边 | 常见低边 | 常见低边 |
| **驱动特点** | 常需要高边驱动 / Bootstrap | 低边驱动，相对简单 | 低边驱动，相对简单 |
| **输出电容作用** | 滤波、瞬态储能 | 储能 + 滤波 + 接力供电 | 储能 + 滤波 + 接力供电 |
| **MOSFET理想耐压** | 约 $V_{in}$ | 约 $V_o$ | 约 $V_{in}+|V_o|$ |
| **代表相关拓扑** | 正激、同步 BUCK | Boost PFC | Flyback 反激 |
这张表以后复习的时候很有用。
